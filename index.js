...
connection.connect(err =>{
  if (!err) console.log('Connection done with sql')
  else
 console.log('Connection failed with sql', err)
}
)
app.get("/sql" , (req , res) =>{
...
render_home()
      ...
      try {
        ...
        if (check.length > 0) {
          ...
          if (compare_result) {
            ...
            res.status(200).render("login", {
              warn: "Invalid credentials , please check email and password", session: sess
            })}
        } else {
          res.status(200).render("login", {
            warn: "Invalid credentials , please check email and password", session: sess
  ...
  app.post("/signup", (req, res) => {
    ...
    async function saveDetails(obj) {
      try {
        ...
        if (check.length > 0) {
          res.render("login", {
            msg: `"${obj.email}" This email  already exists , login here .`, session: sess
          })

        } else if (obj.password != obj.cpassword) {
          res.render("signup", {
            msg: `Password and Confirm password do not match`, form: obj, session: sess
          })
          ...
          res.render("login", {
            msg: `"${obj.email}" Your account has been created successfully , login here.`, session: sess
          })
        }
      }
      catch (err) {
        console.log(err)
        res.status(200).render("signup", {
          msg: "All fields are mandatory .", form: formData, session: sess
        })
      }
    }
  ...
  app.get("/profile", (req, res) => {
    if (req.session.user) {
      async function fetch_user_data(user_email) {
        try {
          ...
          res.render("account.pug", {
            session: req.session, user: userData, blog : userPost
          })
        }catch {
          err => console.log(err)
        }
      ...
      res.status(200).render("login", {
        warn: "Please login first to view your profile.", session: req.session
      })
    }
  })
  ...
  app.post("/profile/update", upload_profile.single('display-picture'), (req, res) => {
